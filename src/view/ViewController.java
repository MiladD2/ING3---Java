...
public class ViewController {
    ...
    private VBox createCard(Object item) {
        ...
        titleLink.setStyle("-fx-font-weight: bold; -fx-text-fill: white; -fx-padding: 0; -fx-underline: false;");
        titleLink.setOnAction(e -> showDetails(item));

        Label subLbl = new Label(sub); subLbl.setStyle("-fx-text-fill: #b3b3b3; -fx-font-size: 11px;");
    ...
    private void playAlbum(Album a) {
        ...
        currentQueue = new ArrayList<>(a.getMorceaux()); queueIndex = 0;
    ...
    private Hyperlink createEntityLink(Object entity) {
        ...
        Hyperlink link = new Hyperlink(name); link.setStyle("-fx-text-fill: white; -fx-padding: 0;"); link.setOnAction(e -> showDetails(entity)); return link;
    }
    ...
    private void populateMorceauxList(List<Morceau> list) {
        ...
        for (Morceau m : list) {
            ...
            if (isAbonne) {
                Button addBtn = new Button("+"); addBtn.getStyleClass().add("button-secondary");
                addBtn.setStyle("-fx-padding: 2 10; -fx-font-size: 14px;"); addBtn.setOnAction(e -> handleAddToPlaylist(m));
    ...
    public void playMorceau(Morceau m) {
        ...
        try {
            mainController.ecouterMorceau(m); currentMorceau = m; isPlaying = true; currentSeconds = 0;
            if (staticTrackTitle != null) {
                staticTrackTitle.setText(m.getTitre()); staticTrackArtist.setText(m.getInterprete());
                if (staticTotalTimeLabel != null) staticTotalTimeLabel.setText(m.getDureeFormatee());
                if (staticMainPlayButton != null) staticMainPlayButton.setText("⏸");
                if (staticProgressSlider != null) { staticProgressSlider.setMax(m.getDureeSecondes()); staticProgressSlider.setValue(0); }
            }
            updateGuestCounter(); startProgress();
        ...
        progressTimeline = new Timeline(new KeyFrame(Duration.seconds(1), e -> {
            if (isPlaying && currentMorceau != null) {
                ...
                if (currentSeconds > currentMorceau.getDureeSecondes()) {
                    currentSeconds = 0;
                    if (!currentQueue.isEmpty() && queueIndex < currentQueue.size() - 1) { queueIndex++; playMorceau(currentQueue.get(queueIndex)); }
                    else { isPlaying = false; if (staticMainPlayButton != null) staticMainPlayButton.setText("▶"); progressTimeline.stop(); }
                } else {
    ...
    private void configurerListViewPlaylists() {
        ...
        if (user != null) playlistList.getItems().setAll(user.getPlayLists());
        playlistList.setCellFactory(param -> new ListCell<PlayList>() {
            @Override protected void updateItem(PlayList item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) setText(null); else setText(item.getNom() + " (" + item.getNombreMorceaux() + " titres)");
            }
        });
    ...
    private void handleAddToPlaylist(Morceau m) {
        if (mainController == null || !mainController.getSystem().estUnAbonneConnecte()) return;
        Abonne user = mainController.getSystem().getAbonneConnecte();
        List<PlayList> playlists = user.getPlayLists();
        if (playlists.isEmpty()) createNewPlaylistAndAdd(m);
        else {
            List<String> choices = new ArrayList<>();
            for (PlayList p : playlists) choices.add(p.getNom());
            choices.add("+ Créer une nouvelle playlist");
            ChoiceDialog<String> dialog = new ChoiceDialog<>(choices.get(0), choices);
            dialog.setTitle("Ajout Playlist"); dialog.setHeaderText(m.getTitre());
            dialog.showAndWait().ifPresent(c -> {
                if (c.equals("+ Créer une nouvelle playlist")) createNewPlaylistAndAdd(m);
                else { mainController.ajouterMorceauAPlaylist(c, m); showAlert("Succès", "Ajouté !"); }
            });
        }
    }

    private void createNewPlaylistAndAdd(Morceau m) {
        TextInputDialog d = new TextInputDialog("Ma Playlist");
        d.setTitle("Nouvelle Playlist"); d.showAndWait().ifPresent(name -> {
            if (!name.trim().isEmpty()) { mainController.creerPlaylist(name); mainController.ajouterMorceauAPlaylist(name, m); showAlert("Succès", "Ajouté !"); }
        });
    }

    @FXML private void handleCreatePlaylist(ActionEvent event) {
        TextInputDialog d = new TextInputDialog("Ma Playlist");
        d.showAndWait().ifPresent(n -> { if (!n.trim().isEmpty()) { mainController.creerPlaylist(n); configurerListViewPlaylists(); } });
    }
    ...
    @FXML private void handleSearch(ActionEvent event) {
        ...
        if (all || (filterArtiste != null && filterArtiste.isSelected())) for (Artiste art : mainController.rechercherArtistes(q)) r.add(art);
        if (all || (filterGroupe != null && filterGroupe.isSelected())) for (Groupe g : mainController.rechercherGroupes(q)) r.add(g);
    ...
    private void configurerListViewGenerique(ListView<Object> listView) {
        ...
        listView.setCellFactory(p -> new ListCell<Object>() {
            private final HBox root = new HBox(10); private final Label lbl = new Label(); private final Region s = new Region(); private final Button btn = new Button("▶");
            { HBox.setHgrow(s, Priority.ALWAYS); btn.getStyleClass().add("button-primary"); btn.setStyle("-fx-padding: 2 8; -fx-font-size: 10px;"); root.setAlignment(Pos.CENTER_LEFT); root.getChildren().addAll(lbl, s, btn); }
            @Override protected void updateItem(Object item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) setGraphic(null);
                else {
                    boolean isCurr = (item instanceof Morceau && item.equals(currentMorceau));
                    btn.setVisible(item instanceof Morceau); btn.setManaged(item instanceof Morceau);
                    if (item instanceof Morceau) { lbl.setText(((Morceau)item).getTitre() + " - " + ((Morceau)item).getInterprete()); btn.setText((isCurr && isPlaying) ? "⏸" : "▶"); btn.setOnAction(e -> handleItemPlayClick((Morceau)item)); }
                    else if (item instanceof Album) lbl.setText(((Album)item).getTitre());
                    else if (item instanceof Artiste) lbl.setText(((Artiste)item).getNom());
                    else if (item instanceof Groupe) lbl.setText(((Groupe)item).getNom());
                    lbl.setStyle(isCurr ? "-fx-text-fill: #1db954; -fx-font-weight: bold;" : "-fx-text-fill: white;");
                    setGraphic(root);
                }
            }
        });
