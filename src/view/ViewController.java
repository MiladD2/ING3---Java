...
public class ViewController {
    ...
    public void showDetails(Object item) {
        ...
        if (item instanceof Morceau) {
            ...
            if (artistsFlowPane != null) {
                ...
                if (m.getGroupe() != null) { Label gp = new Label("Groupe : "); gp.setStyle("-fx-text-fill: #b3b3b3;"); artistsFlowPane.getChildren().add(new HBox(5, gp, createEntityLink(m.getGroupe()))); }
            }
    ...
    private void showPlaylistDetails(PlayList pl) { loadView("DetailsView.fxml"); if (detailsTitle != null) detailsTitle.setText(pl.getNom()); if (playlistActionsBox != null) { playlistActionsBox.setVisible(true); playlistActionsBox.setManaged(true); } if (detailsRenameButton != null) detailsRenameButton.setOnAction(e -> handleRenamePlaylist(pl)); if (detailsDeleteButton != null) detailsDeleteButton.setOnAction(e -> handleDeletePlaylist(pl)); if (sectionMorceaux != null) { sectionMorceaux.setVisible(true); sectionMorceaux.setManaged(true); } populateMorceauxList(pl.getMorceaux()); if (detailsPlayButton != null) { detailsPlayButton.setOnAction(e -> { if (!pl.getMorceaux().isEmpty()) { currentQueue = new ArrayList<>(pl.getMorceaux()); queueIndex = 0; playMorceau(currentQueue.get(queueIndex)); } }); } }
    ...
    private void populateMorceauxList(List<Morceau> list) { if (morceauxVBox == null || list == null) return; morceauxVBox.getChildren().clear(); boolean isAbonne = mainController != null && mainController.getSystem().estUnAbonneConnecte(); for (Morceau m : list) { HBox row = new HBox(15); row.setAlignment(Pos.CENTER_LEFT); row.getStyleClass().add("list-cell"); row.setStyle("-fx-padding: 10; -fx-background-radius: 5;"); Hyperlink titleLink = new Hyperlink(m.getTitre()); titleLink.setStyle("-fx-text-fill: white; -fx-padding: 0;"); titleLink.setOnAction(e -> showDetails(m)); HBox.setHgrow(titleLink, Priority.ALWAYS); Label duration = new Label(m.getDureeFormatee()); duration.setStyle("-fx-text-fill: #b3b3b3;"); Button p = new Button((m.equals(currentMorceau) && isPlaying) ? "⏸" : "▶"); p.getStyleClass().add("button-primary"); p.setStyle("-fx-padding: 5 10; -fx-font-size: 10px;"); p.setOnAction(e -> handleItemPlayClick(m)); row.getChildren().addAll(p, titleLink, duration); if (isAbonne) { Button addBtn = new Button("+"); addBtn.getStyleClass().add("button-secondary"); addBtn.setStyle("-fx-padding: 2 10; -fx-font-size: 14px;"); addBtn.setOnAction(e -> handleAddToPlaylist(m)); row.getChildren().add(addBtn); } morceauxVBox.getChildren().add(row); } }
